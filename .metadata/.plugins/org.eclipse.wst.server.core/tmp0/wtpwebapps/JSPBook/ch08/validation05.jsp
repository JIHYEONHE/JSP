...
function checkMember() {
	...
	let regExpName = /^[가-힣]*$/; //이름은 한글만 사용 가능 ^시작, $끝 , *0이상 반복
	let regExpPasswd = /^[0-9]*$/; //비밀번호는 숫자만 사용 가능
	let regExpPhone = /^\d{3}-\d{3,4}-\d{4}$/; //전화번호 형태만 가능 \d{3}:숫자3자 \d{3,4}:숫자3이상4이하
	let regExpEmail 
	= /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i; //이메일 형태만 가능   i:ignore(대소문자 구별안함)
	//폼 object
	let form = document.Member;
	let id = form.id.value; //문자형
	...
	//regular expression(정규표현식)의 메소드 중  test() => boolean으로 리턴
	//아이디 체크 !regExpId.test(id) : false (통과못했다면)
	if(!regExpId.test(id)){
	...
	if(!regExpName.test(name)){
		alert("이름은 한글만 입력해주세요!"); //smith(x)
		return;
	}
	//비밀번호 체크(비밀번호는 숫자만 사용가능)
